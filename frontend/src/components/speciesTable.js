...
function SpeciesTable() {
  ...
  // save after editing species
  const handleSave = () => {
    const updatedData = data.map((item) => {
      if (item.scientificName === tempData.scientificName) {
        return { ...tempData };
      }
      return item;
    });
  ...
  // delete row
  const handleDeleteRow = (scientificName) => {
    setData((prev) =>
    ...
    if (searchTerm.trim() === "") {
      ...
      const results = data.filter((item) => {
        ...
        const commonNameMatch = Array.isArray(item.commonName)
          ...
          : terms.every((term) =>
            item.commonName.toLowerCase().includes(term)
          );
  ...
  return (
    <div style={{ width: "100%" }}>
      ...
      <Table>
        ...
        <TableBody>
          {displayData &&
            displayData.sort((a, b) => a.scientificName.localeCompare(b.scientificName)).map((row) => (
                ...
                {editingId === row.scientificName ? (
                  <>
                    ...
                    <TableCell>
                      ...
                      <Button onClick={() => handleDeleteRow(row.scientificName, row)} sx={{ color: "brown" }}>
                        Delete
                      </Button>
                  </TableCell>
                  </>
                    ...
                    <>
                      ...
                      <TableCell>
                      <Button onClick={() => startEdit(row.scientificName, row)}>Edit</Button>
                      <Button onClick={() => handleDeleteRow(row.scientificName, row)} sx={{ color: "brown" }}>Delete</Button>
                    </TableCell>
                  </>
                )}
              </TableRow>
