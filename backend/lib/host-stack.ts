...
export class HostStack extends Stack {
  ...
  constructor(
    ...
    const secret = secretmanager.Secret.fromSecretNameV2(
      this,
      "ImportedSecrets",
      functionalityStack.secret.secretName
    );
    ...
    const WAFwebACL = new wafv2.CfnWebACL(this, "Invasive-Plants-WebACL", {
      ...
      description:
        "This WAF-WebACL run on default rule: AWSManagedRulesAmazonIpReputationList, AWSManagedRulesCommonRuleSet, and AWSManagedRulesKnownBadInputsRuleSet.",
      rules: [
        {
    ...
    const fargateSecurityGroup = new ec2.SecurityGroup(
      ...
      "Fargate-SecurityGroup",
      {
        ...
        description:
          "Security group for FargateCluster; Allow all outbound rule, but only ALBSecurityGroup for inbound.",
      }
    );
    ...
    const ALBSecurityGroup = new ec2.SecurityGroup(this, "ALB-SecurityGroup", {
      ...
      description:
        "Security group for ALB; Allow only fargateSecurityGroup outbound rule, and Https:// and Http:// for inbound.",
    });
    ...
    let CFPrefixListId = this.awsRegions2PrefixListID[Stack.of(this).region]
      ? this.awsRegions2PrefixListID[Stack.of(this).region]
      : prefixListIdParam.valueAsString;
    ...
    ALB.logAccessLogs(
      s3.Bucket.fromBucketName(this, "ALBLogID", functionalityStack.bucketName)
    );
    ...
    const containerDefinition = ecsTask.addContainer("taskContainer", {
      ...
      secrets: {
        REACT_APP_USERPOOL_ID: ecs.Secret.fromSecretsManager(
          secret,
          "REACT_APP_USERPOOL_ID"
        ),
        REACT_APP_USERPOOL_WEB_CLIENT_ID: ecs.Secret.fromSecretsManager(
          secret,
          "REACT_APP_USERPOOL_WEB_CLIENT_ID"
        ),
        REACT_APP_REGION: ecs.Secret.fromSecretsManager(
          secret,
          "REACT_APP_REGION"
        ),
        // REACT_APP_X_API_KEY: ecs.Secret.fromSecretsManager(
        //   secret,
        //   "REACT_APP_X_API_KEY"
        // ),
      },
    ...
    const ALBFargateService =
      new ecspatterns.ApplicationLoadBalancedFargateService(
        this,
        "Host-With-LoadBalancer-Dashboard",
        {
          cluster: cluster,
    ...
    const origin_ALB = new origins.LoadBalancerV2Origin(
    ...
    const CFDistribution = new cloudfront.Distribution(
      this,
      "InvasivePlants-CloudFront-Distribution",
      {
        defaultBehavior: {
          origin: origin_ALB,
