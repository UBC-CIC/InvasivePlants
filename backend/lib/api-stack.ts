...
export class APIStack extends Stack {
  ...
  constructor(
     ...
     */
    const IL_InvasiveSpecies = new lambda.Function(
      this,
      "IntegLambInvasiveSpecies",
      {
        runtime: lambda.Runtime.NODEJS_16_X, // Execution environment
        code: lambda.Code.fromAsset("lambda"), // Code loaded from "lambda" directory
    ...
    // Change Logical ID to match the one decleared in YAML file of Open API
    const cfnLambda_invasiveSpecies = IL_InvasiveSpecies.node
     ...
     */
    const IL_alternativeSpecies = new lambda.Function(
      this,
      "IntegLambAlternativeSpecies",
      {
        runtime: lambda.Runtime.NODEJS_16_X, // Execution environment
        code: lambda.Code.fromAsset("lambda"), // Code loaded from "lambda" directory
    ...
    // Change Logical ID to match the one decleared in YAML file of Open API
    const cfnLambda_alternativeSpecies = IL_alternativeSpecies.node
      .defaultChild as lambda.CfnFunction;
    cfnLambda_alternativeSpecies.overrideLogicalId(
      "IntegLambAlternativeSpecies"
    );
    ...
    // Change Logical ID to match the one decleared in YAML file of Open API
    const cfnLambda_saveList = IL_saveList.node
      .defaultChild as lambda.CfnFunction;
    ...
    // Add the policy to the Lambda function's policy to access S3 putObjectAcl
    IL_getSignedURL.addToRolePolicy(
      iam.PolicyStatement.fromJson({
        Effect: "Allow",
        Action: [
          "s3:putObjectAcl",
    ...
    // Change Logical ID to match the one decleared in YAML file of Open API
    const cfnLambda_getSignedURL = IL_getSignedURL.node
      .defaultChild as lambda.CfnFunction;
    ...
    // Change Logical ID to match the one decleared in YAML file of Open API
    const apiGW_authorizationFunction_user = authorizationFunction_user.node
      .defaultChild as lambda.CfnFunction;
